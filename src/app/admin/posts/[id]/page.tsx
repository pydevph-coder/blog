...
async function updatePost(id: string, formData: FormData) {
  ...
  if (!title || (!contentHtml && !contentMarkdown)) {
    throw new Error("Missing required fields");
  }

  // Get existing post to preserve content if needed
  ...
  if (contentHtml && !contentMarkdown) {
    // HTML provided, no markdown - use HTML directly, preserve existing markdown
    finalContentMarkdown = existingPost.contentMarkdown || "";
    ...
    finalContentHtml = existingPost.contentHtml || "";
    finalContentMarkdown = existingPost.contentMarkdown || "";
  }
  ...
  await createAuditLog("update", "post", id, `Updated post: ${title}`);
...
export default async function EditPostPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const selectedTagIds = post.tags.map((pt) => pt.tag.id);
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Update post details</p>
          </div>
          <Link
            href="/admin/posts"
        ...
        <form action={updatePost.bind(null, id)} className="space-y-6 bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <div>
